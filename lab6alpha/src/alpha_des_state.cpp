...
void DesStatePublisher::initializePublishers() {
    ...
    twist_commander_ = nh_.advertise<geometry_msgs::Twist>("cmd_vel", 1);
}
...
bool DesStatePublisher::appendPathQueueCB(lab6alpha::pathRequest& request, lab6alpha::pathResponse& response) {
...
void DesStatePublisher::pub_next_state() {
    ...
    if (e_stop_reset_) {
        ...
        if (motion_mode_ != E_STOPPED && motion_mode_ != HALTING) {
        ...
        //OK...want to resume motion from e-stopped mode;
            ...
            ROS_INFO("switch to DONE_W_SUBGOAL");
        }
    }
    ...
    twist_commander_.publish(current_des_state_.twist.twist);
    //desired_state_publisher_.publish(halt_state_);
}
